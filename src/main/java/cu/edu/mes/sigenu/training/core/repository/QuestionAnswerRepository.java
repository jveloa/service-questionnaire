...
public interface QuestionAnswerRepository extends JpaRepository<QuestionAnswer,Integer>, JpaSpecificationExecutor<QuestionAnswer> {
     ...
     @Query(value = "select count(qa.id) from student_answer sa "
            ...
            + "where date_part('year',qs.done_date) = ?1 and q.id = ?2 and a.id = ?3 "
            + "and q.group_question_id = 11  and qu.id = ?4", nativeQuery=true)
    int totalAnswerByQuestion(int year, int questionId, int answerId, int questionnaireId);
    ...
    List<QuestionAnswer> findQuestionAnswerByQuestion(int questionId, int questionnaireId);
    ...
    @Query(value = "select count(qa.id) from student_answer sa "
            ...
            + "inner join questionnaire_student qs on qs.student_sigenu_id = sa.student_sigenu_id "
            + "inner join questionnaire qu on qu.id = qs.questionnaire_id "
            + "where date_part('year',qs.done_date) = ?1 and qu.id = ?3 and q.id = 56 "
            + "and a.id = ?2 and q.group_question_id = 4 ", nativeQuery=true)
    int totalAnswerByQuestionByStudyHours(int year, int answerId, int questionnaireId);
    ...
    @Query(value = "select count(qa.id) from student_answer sa "
            ...
            + "and a.id = 51 and q.group_question_id = 6 ", nativeQuery=true)
    int totalAnswerByQuestionByStudentsUjc(int year, int questionnaireId);
